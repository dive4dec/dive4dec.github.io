"use strict";
(self["webpackChunkjupyter_divewidgets"] = self["webpackChunkjupyter_divewidgets"] || []).push([["vendors-node_modules_codemirror_view_dist_index_js"],{
...
/* harmony export */ });
/* harmony import */ var _codemirror_state__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @codemirror/state */ "webpack/sharing/consume/default/@codemirror/state/@codemirror/state?8fad");
...
function moveVisually(line, order, dir, start, forward) {
    ...
    let nextIndex = (0,_codemirror_state__WEBPACK_IMPORTED_MODULE_0__.findClusterBreak)(line.text, startIndex, indexForward);
    movedOver = line.text.slice(Math.min(startIndex, nextIndex), Math.max(startIndex, nextIndex));
    if (nextIndex != span.side(forward, dir))
        return _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.EditorSelection.cursor(nextIndex + line.from, indexForward ? -1 : 1, span.level);
    let nextSpan = spanI == (forward ? order.length - 1 : 0) ? null : order[spanI + (forward ? 1 : -1)];
    if (!nextSpan && span.level != dir)
        return _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.EditorSelection.cursor(forward ? line.to : line.from, forward ? -1 : 1, dir);
    if (nextSpan && nextSpan.level < span.level)
        return _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.EditorSelection.cursor(nextSpan.side(!forward, dir) + line.from, forward ? 1 : -1, nextSpan.level);
...
function moveVertically(view, start, forward, distance) {
    ...
    for (let extra = 0;; extra += 10) {
        ...
        if (curY < rect.top || curY > rect.bottom || (dir < 0 ? pos < startPos : pos > startPos))
            return _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.EditorSelection.cursor(pos, start.assoc, undefined, goal);
    }
}
    ...
    if (start > -1) {
        ...
        if (diff) {
            ...
            change = { from: from + diff.from, to: from + diff.toA,
                insert: _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.Text.of(reader.text.slice(diff.from, diff.toB).split(LineBreakPlaceholder)) };
        }
        ...
        if (head != sel.head || anchor != sel.anchor)
            newSel = _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.EditorSelection.single(anchor, head);
    }
    if (!change && !newSel)
        return false;
...
class EditorView {
    ...
    */
    ...
    dispatch(...input) {
        this._dispatch(input.length == 1 && input[0] instanceof _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.Transaction ? input[0]
    ...
    */
    ...
    mountStyles() {
        ...
        style_mod__WEBPACK_IMPORTED_MODULE_1__.StyleModule.mount(this.root, this.styleModules.concat(baseTheme$1).reverse());
    }
    readMeasured() {
    ...
    if (scopeObj) {
        ...
        if (isChar && (event.shiftKey || event.altKey || event.metaKey || charCode > 127) &&
            ...
            if (runFor(scopeObj[prefix + modifiers(baseName, event, true)]))
                return true;
            else if (event.shiftKey && w3c_keyname__WEBPACK_IMPORTED_MODULE_2__.shift[event.keyCode] != baseName &&
                runFor(scopeObj[prefix + modifiers(w3c_keyname__WEBPACK_IMPORTED_MODULE_2__.shift[event.keyCode], event, false)]))
                return true;
        }
...
function wrappedLine(view, pos, inside) {
    let range = _codemirror_state__WEBPACK_IMPORTED_MODULE_0__.EditorSelection.cursor(pos);
...
//# sourceMappingURL=vendors-node_modules_codemirror_view_dist_index_js.f0d7fdd0d41299a0f330.js.map